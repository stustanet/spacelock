...
// (c) 2015 Michael Wendland <accounts@michiwend.com>
// (c) 2019 Stefan Huber <suhu@stusta.de>
        ...
        gr = 0) //gr=0 -> both; gr=1 -> big gear; gr=2 -> small gear
{
    ...
    rotate([0,0,90])
        translate ([0,0,pitch_apex1+20])
        {
            ...
            if(gr==0){
                // kleines zahnrad (wenns eingebaut is)
            ...
            if(gr==2){
                // kleines zahnrad, alleinestehend.
                // sollte eigentlich mit dem gr==0-block vereint werden.
                translate([0,0,-pitch_apex2])
                    bevel_gear (
                            face_width=10,
                            gear_thickness = 2,
...
module zahnrad(groesse=3) {
    ...
    if(groesse==0||groesse==1){ //beide zahnräder malen (0) oder nur das große
        difference(){
            difference(){
                union(){
                    difference(){
                        union(){
                            ...
                            // mittelstück vom großen zahnrad
